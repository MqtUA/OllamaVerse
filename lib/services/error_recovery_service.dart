...
class ErrorRecoveryService {
  ...
  final _errorStateController =
      StreamController<Map<String, ErrorState>>.broadcast();
  ...
  Stream<Map<String, ErrorState>> get errorStateStream =>
      _errorStateController.stream;

  /// Get current error states for all services
  Map<String, ErrorState> get currentErrorStates =>
      Map.unmodifiable(_serviceErrors);

  /// Check if a service has active errors
  bool hasServiceError(String serviceName) =>
      _serviceErrors.containsKey(serviceName);

  /// Get error state for a specific service
  ErrorState? getServiceError(String serviceName) =>
      _serviceErrors[serviceName];
  ...
  Future<T?> handleServiceError<T>(
    ...
    try {
      ...
      AppLogger.error(
          'Error during recovery for service: $serviceName', recoveryError);
      return null;
    }
  }
    ...
    try {
      ...
      final result = await ErrorHandler.executeWithTimeout(
        () => ErrorHandler.executeWithRetry(
          operation,
          operationName:
              '$serviceName${operationName != null ? '.$operationName' : ''}',
  ...
  void _updateErrorTracking(String serviceName, Object error) {
    ...
    if (lastErrorTime != null &&
        now.difference(lastErrorTime) > _errorResetWindow) {
  ...
  Future<T?> _attemptRecovery<T>(
    ...
    try {
      ...
      AppLogger.error(
          'Recovery attempt failed for service: $serviceName', error);
      return null;
    }
  }
